...
$editid = $_GET['id'];
//Fetch Data
$editq = mysqli_query($connection, "select * from hotels where hotel_id='{$editid}'") or 
...
if($_POST){
    ...
    if(!empty($_FILES["himage"]["name"])) { 
        ...
        if(in_array($fileType, $allowTypes)){ 
            $himage = $_FILES['himage']['tmp_name']; 
            $imgContent = addslashes(file_get_contents($himage)); 
         
            // Insert image content into database 
            //$insert = $db->query("INSERT into package (image) VALUES ('$imgContent')"); 
             
             $q1 = mysqli_query($connection, "update hotels set himage = '{$imgContent}' where hotel_id='{$editid}'") or 
    die(mysqli_error($connection));

            if($q1){ 
                $status = 'success'; 
                $statusMsg = "File uploaded successfully."; 
                echo "<script>alert('Record Updated');window.location='hotels.php';</script>";
    ...
    $hdes_country = $_POST['hdes_country'];
    $hdes_state = $_POST['hdes_state'];
    $hdes_city = $_POST['hdes_city'];
    $hhighlights = $_POST['hhighlights'];
    $charges_per_night = $_POST['charges_per_night'];
    //$himage = $_POST['himage'];
    
        
    
    $q = mysqli_query($connection, "update hotels set hotel_name='{$hotel_name}',
    hdes_country='{$hdes_country}',hdes_state='{$hdes_state}', hdes_city= '{$hdes_city}', hhighlights = '{$hhighlights}',
    charges_per_night = '{$charges_per_night}' where hotel_id='{$editid}'") or 
    die(mysqli_error($connection));
    if($q){
        echo "<script>alert('Record Updated');window.location='hotels.php';</script>";
    }
        ...
        <div class="container mt-4 mb-4 p-3 d-flex justify-content-center">
            <div class="card p-4">
                ...
                <div class=" image d-flex flex-column justify-content-center align-items-center"> 
                    <div class="d-flex flex-row justify-content-center align-items-center gap-2"> <span class="idd1">Hotel Name:<input type="varchar" value="<?php echo $editrow['hotel_name'] ?>" name="hotel_name"/></div><br/>
                    <div class="d-flex flex-row justify-content-center align-items-center gap-2"> <span class="idd1">Country:<input type="varchar" value="<?php echo $editrow['hdes_country'] ?>"name="hdes_country"/></span></div> <br/>
                    <div class="d-flex flex-row justify-content-center align-items-center gap-2"> <span class="idd1">State:<input type="varchar" value="<?php echo $editrow['hdes_state'] ?>" name="hdes_state"/> </div><br/>
                    <div class="d-flex flex-row justify-content-center align-items-center gap-2"> <span class="idd1">City:<input type="varchar" value="<?php echo $editrow['hdes_city'] ?>" name="hdes_city"/> </div><br/>
                    <div class="d-flex flex-row justify-content-center align-items-center gap-2"> <span class="idd1">Highlights:<input type="varchar" value="<?php echo $editrow['hhighlights'] ?>" name="hhighlights"/> </div><br/>
                    <div class="d-flex flex-row justify-content-center align-items-center gap-2"> <span class="idd1">Per Night Rate:<input type="number" value="<?php echo $editrow['charges_per_night'] ?>" name="charges_per_night"/></div><br/>
                     <div class="d-flex flex-row justify-content-center align-items-center gap-2"> <span class="idd1">Image:<input type="file" value="" name="himage"/> </div><br/>
                    


                    <input type="submit" value="Edit" class="btn1 btn-dark">
                    <a href="hotels.php" style="color: black; font-size: 20"> Cancel </a>
